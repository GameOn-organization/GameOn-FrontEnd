...
export default function SwipeTabsLayoutWithVisualFeedback() {
  ...
  useEffect(() => {
    ...
    const index = TAB_ROUTES.findIndex(tab => tab.name === currentSegment);
  ...
  const navigateToTab = (index: number) => {
    if (index >= 0 && index < TAB_ROUTES.length && index !== currentTabIndex) {
  ...
  const panGesture = Gesture.Pan()
    ...
    .onEnd((event) => {
      ...
      const threshold = screenWidth * 0.2; // 20% da largura da tela
      const velocity = event.velocityX;
      
      // Considera tanto a distância quanto a velocidade do gesto
      const shouldNavigate = Math.abs(event.translationX) > threshold || Math.abs(velocity) > 500;
      
      if (shouldNavigate) {
        if (event.translationX > 0 && currentTabIndex > 0) {
          // Deslizar para a direita - tab anterior
          runOnJS(navigateToTab)(currentTabIndex - 1);
        } else if (event.translationX < 0 && currentTabIndex < TAB_ROUTES.length - 1) {
